...
import { callsApi, agentApi, twilioApi } from '../services/api';
...
const OutboundPage: React.FC = () => {
  ...
  const [csvNumbers, setCsvNumbers] = useState(''); // Multi-number support for batching!
  ...
  const handleCreateBatchCall = async (e: React.FormEvent) => {
    ...
    // Trigger calls sequentially or concurrently
    for (const num of numbersToDial) {
      try {
        ...
        toast.error(`Error dialing ${num}: ${err.response?.data?.detail || 'Bridge failure'}`);
      }
    }
  ...
  const filteredCalls = calls.filter(call => {
    ...
    return call.to_number?.includes(term) || 
  ...
  return (
    <div className="space-y-6 text-[var(--text-primary)] relative pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 relative z-10">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[28px] font-extrabold tracking-tight text-[var(--text-primary)]">Batch Calling</h1>
          </div>
          <p className="text-xs text-[var(--text-muted)] mt-1 font-medium ml-[52px]">Manage large-scale outbound voice campaigns and dispatch agents.</p>
        </div>
        <button
      ...
      <div className="relative z-10">
        {loading ? (
          ...
          <div className="w-full flex flex-col items-center justify-center py-20 px-4 border border-[var(--border)] rounded-2xl bg-[var(--surface-secondary)]/10 text-center">
            ...
            <h3 className="text-sm font-bold text-[var(--text-primary)] uppercase tracking-wider mb-1">No batch calls found</h3>
            <p className="text-xs text-[var(--text-muted)] max-w-sm mb-6">No batch calls found for the workspace. Create a call dispatch to trigger outbound campaigns.</p>
            <button
              onClick={() => setIsDrawerOpen(true)}
                    ...
                    return (
                      <tr key={call.id} className="hover:bg-[var(--surface-secondary)]/15">
                        ...
                        <td className="px-6 py-4 text-[var(--text-muted)]">
                          {call.started_at ? new Date(call.started_at).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' }) : 'Pending'}
                        </td>
                        <td className="px-6 py-4 font-semibold text-[var(--text-secondary)]">
                          {call.duration_seconds ? `${Math.round(call.duration_seconds)}s` : '0s'}
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-2 py-0.5 border text-[10px] rounded font-bold uppercase tracking-wider ${statusColor}`}>
                            {call.status}
                          </span>
                        </td>
                      </tr>
      ...
      {/* RIGHT SIDE CREATE BATCH CALL DRAWER */}
      {isDrawerOpen && (
          ...
          <div className="relative w-full max-w-[420px] h-full bg-[var(--surface)] border-l border-[var(--border)] shadow-2xl flex flex-col z-10">
            ...
            <form onSubmit={handleCreateBatchCall} className="flex-1 overflow-y-auto px-6 py-5 space-y-5 custom-scrollbar">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider ml-0.5">Select Agent (Persona)</label>
                <select
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider ml-0.5">Single Destination Number</label>
                <input
                  type="text"
              ...
              {/* Batch Import Field */}
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider ml-0.5">Batch Numbers (CSV / Line-separated)</label>
                <textarea
                  rows={4}
                ...
                />
                <p className="text-[9px] text-[var(--text-muted)] font-medium leading-relaxed">Enter one E.164 formatted number per line or separated by commas.</p>
              </div>

              {/* Dialing Alert Banner */}
              <div className="flex items-start gap-2.5 p-3.5 bg-[var(--surface-secondary)]/20 border border-[var(--border)] rounded-xl">
                ...
                <div className="space-y-0.5">
                  <span className="text-[10px] font-extrabold uppercase tracking-wider text-[var(--text-primary)]">Automated Outbound Dispatch</span>
                  <p className="text-[9px] text-[var(--text-muted)] leading-relaxed">Ensure Twilio credentials and voice gateway are online before running batch campaigns.</p>
                </div>
              </div>

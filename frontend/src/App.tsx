        ...
        <header className="h-16 px-4 md:px-6 flex items-center justify-between border-b border-white/5 bg-zinc-950/80 backdrop-blur-xl sticky top-0 z-10">
          <div className="flex items-center gap-3 md:gap-5">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-lg bg-zinc-900 border border-white/5 flex items-center justify-center text-[9px] font-black text-primary overflow-hidden uppercase">
                  {user?.full_name?.split(' ').map(n => n[0]).join('') || 'ID'}
              </div>
              <span className="text-[13px] font-bold text-zinc-200 tracking-tight line-clamp-1">{user?.full_name || user?.email}</span>
            </div>
          </div>
